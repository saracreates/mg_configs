...
def generate_analysis_sub():
    ...
    input_base = "/eos/user/s/saaumill/Hgamma_fullsim_data/ddsim_higgsgamma"
    # output_base = "/afs/cern.ch/work/s/saaumill/public/tmp_fullsim_output/reco_higgsgamma/"
    output_base = "/eos/user/s/saaumill/Hgamma_fullsim_data/reco_higgsgamma/"
    
    # Prepare the header of the file
    header = """# run commands for analysis,

# here goes your shell script
executable    = submit_reco_withJetTagger.sh
...
"""
    ...
    with open("reco_withTagger.sub", "w") as file:
        file.write(header)
        file.write(content)
